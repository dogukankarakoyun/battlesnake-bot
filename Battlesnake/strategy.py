...
#Probiere mehrere Pfade, damit es auch auf Replit funktioniert
possible_paths = [
...
if ml_path is None:
    debug(
        f"[ERROR] LightGBM directory not found. Tried paths: {[str(p) for p in possible_paths]}"
    )
    ml_path = Path.cwd() / "LightGBM"  # Fallback
...
def classify_move_quality(proba_dict,
                          actual_move,
                          flood_fill_space_after=None):
    """
...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    if best_move is None:
        ...
        try:
            ...
            food_goals = set(
                [Cell(f["x"], f["y"]) for f in game_state["board"]["food"]])
            valid_goals = [
                goal for goal in food_goals
                if goal not in path_solver.forbidden_cells
            ]
            if not valid_goals:
                valid_goals = list(path_solver.neighbors(my_head))
            astar_result = path_solver.astar(
                my_head, valid_goals[0]) if valid_goals else None
    ...
    if best_move is None:
        ...
        try:
            if ml_model:
                ...
                debug(
                    f"[ML] Vorhersage: {ml_prediction}, Wahrscheinlichkeiten: {proba_dict}"
                )
